...
let config=JSON.parse(await readFile('monitoring.config.json','utf8'));if(process.env.CONFIG_API_URL){try{const r=await fetch(`${process.env.CONFIG_API_URL.replace(/\/$/,'')}/config`);if(r.ok)config=await r.json();else throw new Error(`config_api_${r.status}`)}catch(e){console.error(`Remote config unavailable, using local config: ${e.message}`)}}
...
async function retryStage({stage,script,args,accept,attempts=config.retryAttempts||3}){const failures=[];for(let attempt=1;attempt<=attempts;attempt++){try{const value=await run(script,args,config.stageTimeoutMs||60000);if(accept(value))return {ok:true,value,attempt,failures};failures.push({attempt,result:value.status||value.result?.status||'rejected',diagnostic:stage==='reviews'?{sourceUrl:value.sourceUrl,navigationError:value.navigationError,reviewLink:value.reviewLink,observedPage:value.observedPage,validation:value.validation}:undefined})}catch(e){failures.push({attempt,error:e.message})}if(attempt<attempts)await sleep((config.retryDelaysMs||[30000,120000])[attempt-1]||120000)}return {ok:false,stage,failures}}
